...
wrangler secret put BAZAAR_BOOTSTRAP_SECRET   # gates the CDP-settlement path used to seed Bazaar listings
```

> **pnpm 11**: pnpm-specific settings live in `pnpm-workspace.yaml` (the `pnpm` field in `package.json` is no longer read) — `allowBuilds` (replaces `onlyBuiltDependencies`) plus `peerDependencyRules`/`allowedDeprecatedVersions` (no `overrides` remain — the `fast-uri` pin was dropped once ajv resolved past the advisory on its own). `LOG_LEVEL` is a `[vars]` entry in `wrangler.toml`. `mcp-server/` is a **separate** npm package (own lockfile/build), not part of the workspace.
...
- **`src/config.ts`** — Centralized pricing, network/facilitator config, cache settings, viewport bounds, timeouts. All prices defined here. **Pricing floor rule:** every SerpAPI-backed endpoint (search family, verticals, transcript) costs $0.009–$0.015 upstream per call depending on plan tier — never price those below $0.015. `/answer` bundles a SerpAPI call + a capped Haiku call (~$0.026 worst case) — keep its price ≥ 2x that. **Two pricing tiers exist and must not be confused:** endpoints with a paid upstream (SerpAPI/Anthropic) carry a real cost floor, but native-fetch endpoints (`/fetch/basic`, `/contents`, `/map`, `/crawl`, `/batch/fetch`, `/pdf`) cost ~$0.000001-0.00002 per call because Workers bills $0.30/M requests + $0.02/M CPU-ms and **does not bill subrequests at all** — those are priced to win on price, not to clear a floor. Browser Rendering endpoints (`/fetch/pro`, `/screenshot`) sit between at $0.09/browser-hour. Per-unit resolvers format to **4 decimals**: at a sub-$0.001 unit price, 3 decimals rounds odd counts up and overcharges. Prices are duplicated by hand in `mcp-server/src/index.ts` (separate package) — `tests/unit/mcp-package-prices.test.ts` pins them in sync.
...
- Facilitator selection happens at runtime in `getResourceServer()` (`src/middleware/payment.ts`) based on env: `NETWORK=base-sepolia` (or a `FACILITATOR_URL` containing `x402.org`) → x402.org testnet facilitator; else if `CDP_API_KEY_ID`/`CDP_API_KEY_SECRET` are set → **PayAI primary + CDP fallback** (two `HTTPFacilitatorClient`s; PayAI wins precedence for shared scheme/network combos because the CDP facilitator has known Base-mainnet gas-estimation issues — coinbase/x402#1065 — so CDP is kept only as a fallback for schemes PayAI doesn't advertise); else → PayAI only. The chosen branch is logged at init (`wrangler tail` shows e.g. `Facilitator: payai (primary) + cdp (fallback)`). **Multi-network:** `config.ts` no longer hardcodes a network list. `evmNetwork(env)`, `svmNetwork(env)` and `supportedNetworks(env)` derive it, so a network can never be advertised on a discovery surface while the wall cannot take money on it. EVM is always on; **Solana is gated on `PAY_TO_ADDRESS_SVM`** and `getResourceServer` registers `ExactSvmScheme` only when it is set — registering a network the facilitator does not advertise on `/supported` leaves `initialize()` with no supported kind and `buildPaymentRequirements` then throws for every request on the route. Solana needs no SOL and no RPC: PayAI settles it with a sponsored `feePayer` from its `/supported` `extra` (`CjNFTjvBhbJJd2B5ePPMHRLx1ELZpa8dwQgGL727eKww` on mainnet), and the USDC mint defaults from `@x402/svm`. The payout address **must already have a USDC associated token account** — the SVM `exact` spec makes the facilitator verify the destination exists. Discovery reports CAIP-2 ids (`eip155:8453`), not `"base"`, so an agent can match them against `accepts[].network`.
...
### Free Previews (`src/services/previews.ts`, `POST /preview`)
Answers "is this worth paying for?" before the 402 — the measured ecosystem failure is agents probing and never buying. Returns the price for the caller's own body plus a recorded response sample. **Cost rule: a preview must never cost us money.** Only `LIVE_PREVIEW_ENDPOINTS` (no paid upstream — native fetch only) may run a real truncated preview; SerpAPI/Anthropic-backed endpoints always return the recorded sample. Tests assert both that every `PAID_ENDPOINTS` entry has a sample and that no live-preview endpoint has a paid upstream — adding a paid endpoint without a preview fails the build.

### ERC-8004 (`src/services/erc8004.ts`, `src/middleware/receipt.ts`)
WebLens hosts the **off-chain half** of ERC-8004, which is what the spec permits an operator with no contracts to do: the registration document at `/.well-known/agent-registration.json` (`registration-v1`), per-paid-call receipts at `/receipts/{requestId}` (buyer's `proofOfPayment` evidence; `X-Receipt-Id`/`X-Receipt-Url` headers on paid responses), and `POST /feedback`, which stores a buyer-authored feedback document verbatim and returns the `feedbackURI` + keccak-256 `feedbackHash` pair that `giveFeedback()` takes. Documents are served byte-identically so the buyer can recompute the hash.

**Do not overclaim:** WebLens is not registered on-chain, holds no agent id (`registrations` is deliberately `[]`), and writes to no registry. The receipt `mac` is the same symmetric HMAC as proof-of-context — it is **not** a third-party-verifiable signature. Receipts are only written for calls that were paid *and* served (2xx), so probes cannot fill KV. `viem` supplies keccak-256 and is a **runtime** dependency (also used by `utils/security.ts` for wallet signature verification) — it was previously misfiled under devDependencies.

### Deep Research (`src/services/deep-research.ts`)
`/research/deep` runs plan → search-per-sub-question → fetch/dedupe → cited synthesis in **one synchronous call** (same rationale as `/crawl`). Priced per depth tier via `PRICING.deepResearch`; `DEEP_RESEARCH_TIERS` fixes the bounds that keep cost under price — **changing a tier's `subQuestions`/`maxSources` changes the cost floor, so reprice with it** (a test asserts price > worst-case cost). Synthesis runs on Haiku 4.5 ($1/$5 per MTok) with hard output caps and per-source truncation. Expect **30–60s** wall-clock — document a generous client timeout. Degrades rather than failing a paid request: planning falls back to the raw query, a failed search is skipped, a failed fetch falls back to the search snippet, and non-JSON synthesis is served as prose.
...
- **Environments**: production (Base mainnet + Solana mainnet when `PAY_TO_ADDRESS_SVM` is set, custom domain `api.weblens.dev`) and testnet (Base Sepolia + Solana devnet, `workers_dev = true`)
...
- Uses `@x402/extensions/bazaar` (`declareDiscoveryExtension` in `src/middleware/payment.ts`) — this got WebLens indexed in the **PayAI facilitator discovery catalog** (facilitator.payai.network/discovery/resources, **14** resources as of 2026-09-03; PayAI indexes more broadly than settles alone — 12 of the 14 carry a `lastUpdated` of 2025-11-30, predating any payment, while `/search/places` and `/research/deep` were refreshed within ~15s of their settlements).
- **Why WebLens is absent from the CDP Bazaar — it is not a config defect.** CDP's `POST /platform/v2/x402/validate` (public, needs no API key, and the single best diagnostic here) reports `valid: true`, `simulation: accepted`, **25/25 preflight checks passed** and `index: null` for every endpoint. Per [CDP's seller docs](https://docs.cdp.coinbase.com/x402/seller/get-discovered) there is no registration call: a resource enters the catalog only after **the CDP facilitator itself settles a payment for it**, and indexing is **per-resource** — one payment never covers sibling routes sharing a `payTo`. WebLens keeps PayAI primary in `getResourceServer` (CDP's Base-mainnet `/settle` still fails intermittently — x402-foundation/x402#1065, open, ~40% in our experience), so buyer payments are settled by PayAI and CDP sees none. Verified on-chain: both real payments (0.35 USDC 2026-08-13 = `deepResearch.deep`; 0.045 USDC 2026-08-31 = `places`) emitted EIP-3009 `AuthorizationUsed` and were relayed by **PayAI's** Base relayers `0xc6699d2a…`/`0xb2bd2992…` (attribution: facilitators.x402.watch), batched through Multicall3. Real revenue therefore does *not* imply CDP indexing. (#2112, the old "CDP never emits `EXTENSION-RESPONSES`" indexing bug, was **closed 2026-07-16** — no longer a blocker.)
- **Seeding a CDP listing** (`scripts/bootstrap-payment.ts`, `pnpm run bootstrap-payment`): a request carrying `X-Bazaar-Bootstrap: $BAZAAR_BOOTSTRAP_SECRET` settles through CDP instead of PayAI for that one request (`wantsCdpBootstrap` → `getResourceServer(env, preferCdp)`), so listings can be seeded without pointing any buyer at the unreliable facilitator. Fails closed: no secret configured ⇒ the header is ignored, so the CDP path is unreachable from the internet. The script **plans by default and pays only with `CONFIRM=yes`**, reads each price from that endpoint's own live 402 (never hardcoded) and builds request bodies from the published bazaar `input` example, caps spend via `MAX_PRICE`/`MAX_TOTAL`, retries #1065 failures, and reports the `EXTENSION-RESPONSES` bazaar ack plus the resulting `index` state. Measured plan at `MAX_PRICE=0.02`: 22 endpoints for **$0.1845**; the four `/intel/*` routes add ~$14.75. `/credits/buy` is never seeded (billing action, not a data product).
- **`serviceName` / `tags` / `iconUrl` are `RouteConfig` fields, NOT bazaar-extension fields** (`SERVICE_NAME`/`tagsForPath` in `config.ts`, applied in `createPaymentConfig`). Put them inside `declareDiscoveryExtension()` and they silently never reach the catalog record — the extension preserves unknown keys, so the value still shows up in the 402 header and is echoed back by `/validate` under `bazaarExtension.serviceName`, which looks like confirmation but is not (x402-foundation/x402#2112). Catalogs write the record **at settle time**, so metadata must be deployed *before* seeding or refreshing it costs another payment per route. `catalogIconUrl()` forces https for non-local hosts because `wrangler dev` reports the configured route host over plain http.
- `patches/@x402__extensions@2.24.0.patch` (pnpm patch, wired in `pnpm-workspace.yaml` `patchedDependencies`): `@x402/hono`'s middleware runs an advisory Ajv validation of each route's bazaar extension on cold start, but Ajv compiles schemas via `new Function` — forbidden in workerd — which spammed a paired error+warn per paid route per isolate in production logs. The patch passes `logger: false` to Ajv and treats the codegen-disallowed `EvalError` as "skip validation" instead of a failure. It also guards the **payment-identifier** extension validator (new in 2.20+, same Ajv pattern) — unguarded, a payer attaching a schema to that extension would get a false `valid: false` on the payment path, not just log noise. The erc20-approval-gas-sponsoring Ajv call is facilitator-only code WebLens never reaches — left unpatched. Discovery itself was never affected (the extension is not dropped on validation failure). Upstream tracks this as x402-foundation/x402#3029 with open PR #3093 (same approach: detect the codegen error, skip validation, report once; both still open as of 2026-09-03) — on `@x402/*` upgrades past 2.24.0, re-key the patch (`pnpm patch @x402/extensions@<ver>` → apply the old patch file onto the workspace with `patch -p1` → `pnpm patch-commit`, then delete the old patch file) or drop it if #3093 has shipped.
